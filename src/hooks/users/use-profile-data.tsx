...
import { EditProfileFormSchema, TEditProfileFormSchema } from "@/lib/validators/profile-validations";
import { useMutation, useQueryClient } from "@tanstack/react-query";

import { format } from "date-fns";
import { toast } from "@/hooks/use-toast";
import { setIsEditable, toggleEditButton as toggleEdit } from "@/store/slices/profile-slice";

export const useProfileData = () => {
    const dispatch = useAppDispatch();
    const { profileData, isEditable } = useAppSelector((state) => state.profile);
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: (data: TEditProfileFormSchema) => dispatch(updateProfile(data)).unwrap(),
        mutationKey: ["profile"],
        onSuccess: () => {
    ...
    const addWebsite = () => {
        ...
        form.setValue("websites", [...(form.getValues("websites") || []), websiteInputRef.current?.value]);
